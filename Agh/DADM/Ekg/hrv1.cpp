...
public:
  ...
  return data_input;
	...
	//PO CO JEST TA FUNKCJA ?!?!?!?!?!?!?!?!?!?!??!?!?!?!?!?!?!?!?!??!?!?!?!?! skoro nigdzie jej nie u¿ywamy
		double round;
		if (value<0)
...
//******************************     OBLICZANIE PARAMETRÓW CZASOWYCH     ******************************
	...
	//Obliczanie parametru RR_mean, czyli wartoœci œredniej interwa³ów RR
		double sum = accumulate(temp_vec.begin(),temp_vec.end(),0.0); //suma od pierwszego do ostatniego elementu, ostatni parametr to inicjalizacja
		double mean = sum/temp_vec.size();
		return mean;
	}

	double SDNN(vector<double> &temp_vec){
	//Obliczenie parametru SDNN, czyli odchylenia standardowego interwa³ów RR
		double mean = RR_mean(temp_vec);
		vector<double> temp(temp_vec.size());
		for (unsigned i = 0; i<temp_vec.size(); i++){
			temp[i] = pow((temp_vec[i] - mean),2);
		}
		double sdnn = sqrt((accumulate(temp.begin(),temp.end(),0.0))/temp.size());
		temp.clear();
		return sdnn;
	}
   ...
   //Obliczenie parametru RMSSD, czyli pierwiastka kwadratowego ze œredniej kwadratów ró¿nic pomiêdzy dwoma kolejnymi interwa³ami
	...
	//Wyznaczenie parametru pNN50 - odsetek ró¿nic pomiêdzy interwa³ami RR, które przekraczaj¹ 50ms
		double pnn50 = NN50(temp_vec)/(temp_vec.size()-1);
		return pnn50;
	}

	vector<int> index_300(vector<double> &temp_vec){
	//Wektor indeksów 5 minutowych (przedzi¹³y 300sekund)
		int N = int(floor((accumulate(temp_vec.begin(),temp_vec.end(),0.0))/300)); //;liczba przedzialow 300-sekundoych
...
Lomb_pair Lomb(vector<double> &vec){
    vector<double> time(vec.size()); //wektor czasu
