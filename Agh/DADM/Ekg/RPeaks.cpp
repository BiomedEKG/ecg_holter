...
RPeaksResult* RPeaks::compute(ResultKeeper *rkp){
	...
	*/
	...
	unsigned int sampling_frequency = 360;//rkp->getInput()->GetFs();// 360; to powinno byc pobierane z result keepera
	std::vector <double> signal = (rkp->getECGBaseline()->getFilteredSignal());
	unsigned int siz = rkp->getECGBaseline()->size;
	cout << siz;
	//for(int i = 0; i < siz; i++) {
	//	vector<double> temp = rkp->getECGBaseline()->getFilteredSignal();
	//	signal.push_back(temp[i]);  
	//	cout << i << endl;
	//}
	...
	//switch (RPeaksMethod){
		...
		//case  PANTOMPKINS:
	output = computePT(signal, sampling_frequency, rkp->getECGBaseline()->size);
...
std::vector<unsigned int> RPeaks::computePT(std::vector<double> signal, unsigned int sampling_frequency, unsigned int size){
	...
	double treshold = PTobject.find_cut_level(integral_data, size);
	std::vector<double> tresholded_data = PTobject.treshold_data(integral_data, treshold, size);
	std::vector<unsigned int> R_Peaks = PTobject.find_R(tresholded_data, size);
...
//std::vector<unsigned int> RPeaks::computeHilbert (std::vector<double> signal, unsigned int sampling_frequency, unsigned int size){
//
//    int window_width = 40;
//	PanTompkins PTobject = PanTompkins();
//	std::vector<double> derivative_data = PTobject.differentation(signal, size);
