...
HeartClass::HeartClass(ResultKeeper* rkp){
	...
	this->qrsEnd = vector<double>(wavesData["QRS_END"].begin(), wavesData["QRS_END"].end());
	this->qrsOnset = vector<double>(wavesData["QRS_ONSET"].begin(), wavesData["QRS_ONSET"].end());
	this->signalFromEcg = rkp->getECGBaseline()->getFilteredSignal();
	this->artifactsClass = 	vector<double> (this->qrsEnd.size(), 0);
}
