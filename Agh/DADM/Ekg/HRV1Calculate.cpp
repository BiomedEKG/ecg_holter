...
HRV1Calculate::HRV1Calculate()
{
}
  
std::vector<double> HRV1Calculate::type_change(std::vector <unsigned int> &temp_vec){
	//Zmiana wektora wartoœci typu unsigned int na wektor wartoœciu double.
		std::vector<double> new_vec(temp_vec.size());
		new_vec.clear();
	...
	std::vector<double> HRV1Calculate::inter_RR(std::vector<double> &temp_vec, double fp){ 
	...
	Niniejsza operacja jest konieczne do dalszego wyliczania parametrów.*/
		std::vector<double> inter_rr(temp_vec.size()-1);
	...
	std::vector<double> HRV1Calculate::inter_RRt(std::vector<double> &temp_vec, double fp){
	...
	Tego typu wektor przyda siê podczas obliczania parametrów czêstotliwoœciowych metod¹ periodogramu.*/
		double suma;
		std::vector<double> inter_rrt(temp_vec.size()-1);
...
//******************************     OBLICZANIE PARAMETRÓW CZASOWYCH     ******************************
	...
	double HRV1Calculate::RR_mean(std::vector<double> &temp_vec){
	...
	double HRV1Calculate::SDNN(std::vector<double> &temp_vec){
	//Obliczenie parametru SDNN, czyli odchylenia standardowego interwa³ów RR
		double mean;
		double sdnn;
		std::vector<double> temp(temp_vec.size());
	...
	double HRV1Calculate::RMSSD(std::vector<double> &temp_vec){
   //Obliczenie parametru RMSSD, czyli pierwiastka kwadratowego ze œredniej kwadratów ró¿nic pomiêdzy dwoma kolejnymi interwa³ami
	...
	double HRV1Calculate::NN50(std::vector<double> &temp_vec){
	...
	double HRV1Calculate::pNN50(std::vector<double> &temp_vec){
	...
	std::vector<int> HRV1Calculate::index_300(std::vector<double> &temp_vec){
	//Wektor indeksów 5 minutowych (przedzia³y 300sekund)
		int N;  //liczba przedzialow 5 minutowych
		int i; //indeks calego wektora
		double sum;
		std::vector<int> index(N+1); //wektor indeksów przedzia³ów 5 minutowych (300s)
	...
	double HRV1Calculate::SDANN(std::vector<double> &temp_vec){
	/*Obliczenie parametru SDANN - œrednie odchylenie ze wszystkich œrednich interwa³ów RR w 5 minutowych segmentach 
	ca³ego zapisu*/
		double sdann;
		std::vector<int> tab = index_300(temp_vec);
		std::vector<double> srednia;
	...
	double HRV1Calculate::SDANN_index(std::vector<double> &temp_vec){
	...
	double HRV1Calculate::SDSD(std::vector<double> &temp_vec){
...
//******************************     OBLICZANIE PARAMETRÓW CZÊSTOTLIWOŒCIOWYCH     ******************************
	Lomb_param HRV1Calculate::lomb(std::vector<double> intervals){
	std::vector<double> freqs;
	std::vector<double> int_time;
	...
	double HRV1Calculate::TP_Power(std::vector<double> &temp_vec, std::vector<double> &f){
	...
	double HRV1Calculate::HF_Power(std::vector<double> &temp_vec, std::vector<double> &f){
	...
	double HRV1Calculate::LF_Power(std::vector<double> &temp_vec, std::vector<double> &f){
	...
	double HRV1Calculate::VLF_Power(std::vector<double> &temp_vec, std::vector<double> &f){
	...
	double HRV1Calculate::ULF_Power(std::vector<double> &temp_vec, std::vector<double> &f){	
	...
	double HRV1Calculate::LFHF_Power(std::vector<double> &temp_vec, std::vector<double> &f){
