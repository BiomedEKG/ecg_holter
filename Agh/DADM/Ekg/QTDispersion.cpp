...
*/
		...
		*/
		...
		QTDispResult* qt_disp :: runF( int channel,vector< int> QRSonset,vector< int> tPeak, vector < vector < double > >  SignalsVector, int samplingFrequency)
		{
			...
			const vector <int> &qrsOnset_ = QRSonset; // (*inputwavesMap)["QRS_ONSET"];
			const vector< int> &tEndGlobal_ = tPeak; // (*inputwavesMap)["T_END"];
