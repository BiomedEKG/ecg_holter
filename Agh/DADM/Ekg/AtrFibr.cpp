...
AtrfibrResult* AtrFibr::compute(ResultKeeper* rkp) {
	...
	vector <unsigned int> r_peaks = rkp->getRPeaks()->getRPeaks();//getRpeaksData().getRnum();
	double freq = 360;//rkp.getFreqz();
...
void AtrFibr::createrr(vector<unsigned int>& peaksNum, double freq){
	...
	for(int i = 1; i < peaksNum.size()-1; i++){
...
{
	for (int i = 0; i < (signal.size()-1 - lengthSegment); i++)
	{
...
{
	for (int i = 0; i < signal128.size()-1; i++)
	{
		vector <double> newColumn;
		...
		double calculateMaxValue = maxValue(signal128[i]);//find 8th max value in vector with 128 elements
			...
			for (int j = 0; j < signal128[i].size()-1; j++)
			{
...
{
	for (int i = 0; i < signal112.size()-1; i++)
	{		
		double rmssdValue=0;
		double diff = 0;
		double sum = 0;
		...
		double temp = 0;
			...
			for (int j = 1; j < (signal112[i].size()-1); j++)
			{
...
void AtrFibr::seSegment()
{
	for (int i = 0; i < signal112.size()-1; i++){
		...
		if (step != 0 ){
			...
			//for (int j = 0; j < (signal112[i].size()-1) ; j++)
			for (int j = 0; j < 16 ; j++)
			{
...
{
	for (int i = 0; i < signal128.size()-2; i++) {	// -2 bo inne wektory sa krotsze			
...
vector <unsigned int> AtrFibr::returnDeteckedAF(vector<unsigned int> r_peaks)
{
	...
	for (int i = 0; i < licznikAF.size()-1; i++)//z³y iterator? po rpiksach?
